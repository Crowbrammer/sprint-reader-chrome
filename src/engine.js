    ...
    if (chunkSize == 1) {
      ...
      if (textOnSlide.indexOf("-") !== -1) {
        if (textOnSlide.length == 1) {
          ...
          if (
            lastCharacter !== -1 &&
            textOnSlide[textOnSlide.length - 1] == "-"
          ) {
      ...
      if (madvEnableLongWordHyphenation == "true") {
        ...
        if (textDecoded.length > madvLongWordTriggerCharacterCount) {
          ...
          var splitFirst = $().hyphenateWord(
            textOnSlide,
            language.pattern,
            madvLongWordTriggerCharacterCount
          );
          var splitWord = adjustStringArray(
            splitFirst,
          ...
          for (var x = 0; x < splitWord.length; x++) {
            ...
            if (x == splitWord.length - 1) {
              ...
              if (
                lastTextSegment.length <= madvLongWordCharacterTriggerDoNotJoin
              ) {
                var lastWord =
                  splitTextFirstPass[splitTextFirstPass.length - 1].text;
                splitTextFirstPass[splitTextFirstPass.length - 1].text =
                  lastWord.substring(0, lastWord.length - 1) + lastTextSegment;
              } else {
...
function adjustStringArray(stringArray, minCharCountPerItem, maxItemLength) {
  ...
  for (var i = 0; i < stringArray.length; i++) {
    ...
    if (text.length < minCharCountPerItem && i < stringArray.length) {
      var nextText = "";
      if (typeof stringArray[i + 1] != "undefined")
        nextText = stringArray[i + 1];
...
function splitHyphenatedWord(index, splitText, textItemFirst) {
  ...
  var splitHyphenated = text.split("-").reduce(function(result, current) {
    ...
    if (chrRegEx.test(previous[previous.length - 1])) {
      // && chrRegEx.test(current[0])){
  ...
  if (splitHyphenated.length > 0) {
    for (var i = 0, count = splitHyphenated.length; i < count; i++) {
      ...
      if (i == 0 && splitHyphenated.length == 2) {
        var combinedLength =
          splitHyphenated[0].length + splitHyphenated[1].length;
...
function addCommaPauseAfterCharacter(
  character,
  text,
  textItem,
  splitTextArray
  ...
  if (indexCharacter !== -1) {
    ...
    // We have the character at the start
      ...
      splitTextArray[splitTextArray.length - 1].text =
        splitTextArray[splitTextArray.length - 1].text + ",";
      text = text.slice(1, text.length - 1);
      if (pauseAfterComma == "true") {
        splitTextArray[
          splitTextArray.length - 1
        ].postdelay = pauseAfterCommaDelay;
      }
    }
  }
  return false;
...
function addPeriodPauseAfterCharacter(
  character,
  text,
  textItem,
  splitTextArray
  ...
  if (indexCharacter) {
    ...
    if (indexCharacter == text.length - 1) {
      if (pauseAfterPeriod == "true")
        textItem.postdelay = pauseAfterPeriodDelay;
      return true;
    }
    // We have the character at the start
    else if (indexCharacter == 0) {
      ...
      if (pauseAfterPeriod == "true") {
        splitTextArray[
          splitTextArray.length - 1
        ].postdelay = pauseAfterPeriodDelay;
      }
    }
  }
  return false;
...
function addParagraphPauseAfterCharacter(
  character,
  text,
  textItem,
  splitTextArray
  ...
  if (indexCharacter !== -1) {
    ...
    // We have the character at the start
      ...
      if (pauseAfterParagraph == "true") {
        splitTextArray[
          splitTextArray.length - 1
        ].postdelay = pauseAfterParagraphDelay;
      }
    }
  ...
  for (var i = 0; i < splitText.length; i++) {
    ...
    for (var j = 0; j < chunkSize; j++) {
      if (i + j < splitText.length) {
        ...
        // --------------------------
        // COMMA
        var comma = addCommaPauseAfterCharacter(
          ",",
          text,
          textItem,
          splitTextArray
        ...
        // SEMI-COLON
        var semi = addCommaPauseAfterCharacter(
          ";",
          text,
          textItem,
          splitTextArray
        ...
        // COLON
        var colon = addCommaPauseAfterCharacter(
          ":",
          text,
          textItem,
          splitTextArray
        ...
        // --------------------------
        // PERIOD (Period .)
        var period = addPeriodPauseAfterCharacter(
          ".",
          text,
          textItem,
          splitTextArray
        ...
        var question = addPeriodPauseAfterCharacter(
          "?",
          text,
          textItem,
          splitTextArray
        ...
        // APOSTROPHE (Period !)
        var apostrophe = addPeriodPauseAfterCharacter(
          "!",
          text,
          textItem,
          splitTextArray
    ...
    textItem.pixeloffsettooptimalletter = calculatePixelOffsetToOptimalCenter(
      textItem
    );
...
function getTextArrayBasicTiming(textData) {
  ...
  for (var i = 0; i < textData.length; i++) {
    ...
    if (t.duration < madvBasicMinimumSlideDuration)
      t.duration = madvBasicMinimumSlideDuration;
    // Enable this to get a log of the textItem
...
function getTextArrayWordLengthTiming(textData) {
  ...
  for (var i = 0; i < textData.length; i++) {
    ...
    // Ensure we observe the minimum slide duration setting
    if (t.duration < madvWordLengthMinimumSlideDuration)
      t.duration = madvWordLengthMinimumSlideDuration;
    // Enable this to get a log of the textItem
...
function getTextArrayWordFreqTiming(textData) {
  ...
  for (var i = 0; i < textData.length; i++) {
    ...
    var lowInfo = 4.5;
    var highInfo = 25.6; // this numbers are empirical and are taken from the en-US database.
    ...
    // Ensure we observe the minimum slide duration setting
    if (t.duration < madvWordFreqMinimumSlideDuration)
      t.duration = madvWordFreqMinimumSlideDuration;
...
function playSlideShow() {
  ...
  if (textItemIndex == 1) {
    ...
    if (wordIndex >= textArray.length) {
      ...
      showSlideTime = Math.max(
        1,
        textArray[wordIndex].duration + WPMTimingAdjustmentMS
      );
      // Set the progress
    ...
    if (wordFlicker == "true") {
      {
        setTimeout(function() {
          displayBlankWord();
        }, (showSlideTime * (100 - wordFlickerPercent)) / 100);
      }
    }
    {
      setTimeout(function() {
...
function doWeReverseSlide(textItem) {
  ...
  var result = textItem.textoriginal.replace(
    /\$?[0-9]+(\.[0-9]+\,[0-9]+)?%?/g,
    function(s) {
...
function addStaticFocalToTextItem(textItem, isRightToLeft) {
  ...
  if (isRightToLeft) {
    ...
    var sub2 = text.substring(
      text.length - textItem.optimalletterposition,
      text.length
    );
...
function setSlideShowData(textData) {
  ...
  for (var i = 0; i < textData.length; i++) {
    ...
    slideShowData.totalDurationIncPauses =
      slideShowData.totalDurationIncPauses + d + pd1 + pd2;
  }

  // Adjust the data for WPM adjustments
  var totalAdjustedTiming = WPMTimingAdjustmentMS * textData.length;
  slideShowData.totalDuration =
    slideShowData.totalDuration + totalAdjustedTiming;
  slideShowData.totalDurationIncPauses =
    slideShowData.totalDurationIncPauses + totalAdjustedTiming;

  slideShowData.realWPM = (
    textData.length /
    (slideShowData.totalDuration / 1000 / 60)
  ).toFixed();
  slideShowData.realWPMAllPauses = (
    textData.length /
    (slideShowData.totalDurationIncPauses / 1000 / 60)
  ).toFixed();

  slideShowData.avgDuration =
    slideShowData.totalDuration / slideShowData.totalSegments;
...
function displayStatistics(textData) {
  ...
  document.getElementById("stattotalwords").innerHTML =
    "Total words: <b>" + totalWords + "</b>";
  document.getElementById("stattotalsegments").innerHTML =
    "Total segments (slides): <b>" + s.totalSegments + "</b>";
  document.getElementById("statmaxduration").innerHTML =
    "Maximum slide duration: <b>" +
    s.maxDuration.toFixed() +
    "ms</b> (" +
    getMinAndSecondsString(s.maxDuration) +
    ")";
  document.getElementById("statminduration").innerHTML =
    "Minimum slide duration: <b>" +
    s.minDuration.toFixed() +
    "ms</b> (" +
    getMinAndSecondsString(s.minDuration) +
    ")";
  document.getElementById("statavgperslide").innerHTML =
    "Average slide duration: <b>" +
    s.avgDuration.toFixed() +
    "ms</b> (" +
    getMinAndSecondsString(s.avgDuration) +
    ")";
  document.getElementById("statchunksize").innerHTML =
    "Chunk size (words per slide): <b>" + chunkSize + "</b>";
  document.getElementById("statwpm").innerHTML =
    "Words per minute (WPM): <b>" + WPM + "</b>";
  document.getElementById("stattotaldurationincpause").innerHTML =
    "Total duration (including pauses): <b>" +
    s.totalDurationIncPauses.toFixed() +
    "ms</b> (" +
    getMinAndSecondsString(s.totalDurationIncPauses) +
    ")";
  document.getElementById("stattotalduration").innerHTML =
    "Total duration: <b>" +
    s.totalDuration.toFixed() +
    "ms</b> (" +
    getMinAndSecondsString(s.totalDuration) +
    ")";
}
...
// For testing.
module.exports.splitTextToArray_FirstPass = splitTextToArray_FirstPass;
